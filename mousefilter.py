...
def main(mousefile,humanfile=None,cutoff=0.5):
    ...
    truemouse,truehuman = filter_mouse(mouse,cutoff)
...
def check_inputs(mousefile,humanfile,cutoff):
...
def filter_mouse(mouse,cutoff):
    """ Saves two lists, one of mouse cells and one of human cells with the cell barcodes """
    ####filter by cutoff
    truemouse = mouse[mouse.obs['percent_mouse']>cutoff]
    truehuman = mouse[mouse.obs['percent_mouse']<cutoff]
    ####format genes
